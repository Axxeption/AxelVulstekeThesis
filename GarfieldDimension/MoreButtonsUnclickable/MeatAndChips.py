...
def preprocess(observation):
    if(observation[0] != None):
        ...
        img = observation[125:487, 9:270]  # convert to 210-50x160 input (geel ook al uitgefilterd) anders x = 75
...
def getstate(buttonList):
    ...
    # print(vectorList)
    vectorList = vectorList.reshape([1, 2400])  # dus 3d vorm, dit moet want een tensor mag niet echt 1d zijn!
    # print(vectorList)
    return vectorList
...
env = gym.make('wob.mini.ClickCheckboxes-v0')
...
input_size = 300*8 #ik geef 5 buttons in
howfar =0
buttonList = []
...
with tf.name_scope("fully_connected"):
    W = tf.Variable(tf.random_uniform([2400,8],0,0.02))
...
with tf.Session() as sess:
    ...
    r = [1]
    r[0] = 0
    writer = tf.summary.FileWriter("/home/axel/Documents/TensorFlowLog/MeatPastaMoreButtons/"+ str(datetime.now()))
    ...
    while (observation == [None]):
        ...
        action = [universe.spaces.PointerEvent(x__, y__, 0), universe.spaces.PointerEvent(x__, y__, 1),
                  universe.spaces.PointerEvent(x__, y__, 0), universe.spaces.PointerEvent(200, 500, 0)]
    ...
    while not solutionFound:
        while not solutionFound:
            ...
            while(stuck > 10 or buttonList == None):
                ...
                y__ = 300
                action = [universe.spaces.PointerEvent(x__, y__, 0), universe.spaces.PointerEvent(x__, y__, 1),
                          universe.spaces.PointerEvent(x__, y__, 0), universe.spaces.PointerEvent(250, 500, 0)]
                action = [action for ob in observation]
                observation, reward_n, done_n, info = env.step(action)
                # env.render()
                stuck = 0;
            ...
            if(buttonList != None):
                ...
                buttonList = preprocess(observation)  # ik krijg een buttonlist met 5 buttons terug of ofwel none
                if(action == 7 or action == 8 and not d[0]):
                    x__ = 300
                    y__ = 500
                    waitaction = [universe.spaces.PointerEvent(x__, y__, 0), universe.spaces.PointerEvent(x__, y__, 0),
                                  universe.spaces.PointerEvent(x__, y__, 0)]
