...
def apply_neural_nets(observation_matrix, weights):
    ...
    hidden_layer_values = np.dot(weights['1'], observation_matrix)     #compute hidden layer neuron activations
    hidden_layer_values = relu(hidden_layer_values) #threshold at zero
...
def plotter(x ,y, weights):
    ...
    plt.title('Progress of Pong with downscale')
    plt.savefig('PongWithDownscale.png')
    # plt.show()
    with open('x_file', 'wb') as fp:
        pickle.dump(x, fp)
    with open('y_file', 'wb') as fp:
        pickle.dump(y, fp)
    with open('weightsFile', 'wb') as fp:
         pickle.dump(weights, fp)
    with open('running_reward_file', 'wb') as fp:
        pickle.dump(y[-1], fp)
    with open('episode_number_file', 'wb') as fp:
        pickle.dump(x[-1], fp)
...
def main():
    ...
    #just to keep the weights when shut down PC

    if resume:
        with open('episode_number_file', 'rb') as fp:
            episode_number = pickle.load(fp)
        with open('running_reward_file', 'rb') as fp:
            running_reward = pickle.load(fp)
    ...
    atexit.register(plotter, x, y, weights)
    while True:
        env.render()
        #preprocessing
        processed_observations, prev_processed_observations = preprocess_observations(observation,
                                                                                      ...
                                                                                      input_dimensions)
        #met huidige gewichten de beslissing nemen
        hidden_layer_values, up_probability = apply_neural_nets(processed_observations, weights)

        episode_observations.append(processed_observations)
        #hidden layer values bij houden voor back prop
        episode_hidden_layer_values.append(hidden_layer_values) #hidden layer values are the weights
        ...
        if done:  # an episode finished
            ...
            #als we 10 episodes hebben gedana
            ...
            print('in episode: ', episode_number)
