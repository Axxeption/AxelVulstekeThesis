...
def preprocess(observation):
	if(observation[0] != None):
		...
		if len(buttonList) == 5:
			timeStop = datetime.datetime.now()
			print(" tijd om alle OCR uit te voeren en de Word2Vec te doen: " , timeStop - timeStart)
			return buttonList
		else:
			return None
	return None
...
#
env = gym.make('wob.mini.ChooseList-v0')
...
timedifference.append(timeStart)
timedifference = np.array(timedifference)
with tf.Session() as sess:
	...
	r = [1]
	r[0] = 0
	writer = tf.summary.FileWriter("/home/axel/Documents/TensorFlowLog/CheckButton3/1Layer1-100RandLR0005")
	...
	while not solutionFound:
		while not solutionFound:
			...
			if(buttonList != None):
				...
				timeStop = datetime.datetime.now()
				print(" net na uitvoeren actie" , timeStop - timeStart)
				x__ = 300
				y__ = 300
				...
				env.render()
				timeStop = datetime.datetime.now()
				print(" net na uitvoeren actie 2 (wait actie)", timeStop - timeStart)
				...
				if d[0]:
					...
					if(r[0] > 0):
						...
						numberSolutionFound = 0
					e = 1. / ((num_episode / 100) + 1)
						#start bij .9 --> hoe lang random..? bij 500 episodes ng amar de helft..!
